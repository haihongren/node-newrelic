...
class AwsLambda {
  ...
  patchLambdaHandler(handler) {
    ...
    function wrappedHandler() {
      ...
      const result = handleDTHeaderFromPayload(transaction, event, awsAttributes)
      logger.debug('Proccessed DT header: ', result)
...
function handleDTHeaderFromPayload(transaction, payload, awsAttributes) {
  if (!transaction || typeof payload !== 'object') return

  if ((awsAttributes[EVENT_SOURCE_TYPE_KEY] = 'kinesis')) {
    return handleDTHeaderFromKinesisEvent(transaction, payload)
  }

  return
}

function handleDTHeaderFromKinesisEvent(transactionHandle, payload) {
  const data = payload.Records[0].kinesis.data
  const parsedData = checkPayload(data)
  if (parsedData && parsedData._newrelic) {
    transactionHandle.acceptDistributedTraceHeaders("KINESIS",parsedData["_newrelic"])
    logger.debug('Accepted New Relic DT Trace Header: ', parsedData["_newrelic"])
    return true
  } else {
    logger.error('Unable to parse payload, unable to read trace header')
  }
  return false
}
...
function checkPayload(payload) {
  try {
    ...
    logger.info('Not JSON string. Try Base64 encoding')
  }
  try {
